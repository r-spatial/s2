...
test_that("s2_cap works", {
  ...
  expect_is(s2_cap(cap_ant), "data.frame")
  expect_identical(nrow(s2_cap(cap_ant)), 1L)
  expect_true(s2_cap(cap_ant)$lat < -80)
  expect_true(s2_cap(cap_ant)$angle > 20)

  expect_identical(nrow(s2_cap(s2_data_countries(c("Antarctica", "Netherlands")))), 2L)

  expect_true(s2_cap(s2_data_countries("Netherlands"))$angle < 2)
  expect_true(s2_cap(s2_data_countries("Fiji"))$angle < 2)
})

test_that("s2_lat_lng_rect works", {
  rect_ant <- s2_lat_lng_rect(s2_data_countries("Antarctica"))
  expect_is(rect_ant, "data.frame")
  expect_named(rect_ant, c("lat_lo", "lat_hi", "lng_lo", "lng_hi", "lat_cnt", "lng_cnt"))
  expect_identical(nrow(s2_lat_lng_rect(s2_data_countries(c("Antarctica", "Netherlands")))), 2L)
  ...
  rect_nl <- s2_lat_lng_rect(s2_data_countries("Netherlands"))
  expect_true((rect_nl$lng_hi - rect_nl$lng_lo) < 4)

  rect_fiji <- s2_lat_lng_rect(s2_data_countries("Fiji"))
  expect_true(rect_fiji$lng_hi < rect_fiji$lng_lo)

  rect_multipoint <- s2_lat_lng_rect("MULTIPOINT(-179 0,179 1,-180 10)")
  ...
  rect_linestring <- s2_lat_lng_rect("LINESTRING(-179 0,179 1)")
