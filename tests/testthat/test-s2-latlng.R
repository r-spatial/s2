...
test_that("s2_latlng objects can be created from and converted back to R objects", {
  # in
  expect_is(as_s2_latlng(45, 64), "s2_latlng")
  expect_length(as_s2_latlng(45, 64), 1)
  expect_is(as_s2_latlng(matrix(45, 64, ncol = 2)), "s2_latlng")
  latlng <- as_s2_latlng(45, 64)
  expect_identical(as_s2_latlng(latlng), latlng)
  expect_identical(
    as.data.frame(as_s2_latlng(as_s2_point(1, 0, 0))),
  ...
  expect_length(as_s2_latlng(double(), double()), 0)
  expect_identical(
    as.data.frame(as_s2_latlng(double(), double())),
...
test_that("s2_latlng objects can be printed", {
  expect_output(print(as_s2_latlng(45, 64)), "s2_latlng")
})
