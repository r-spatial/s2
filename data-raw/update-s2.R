...
# the CXXFLAGS=-I../src
headers <- tibble(
  path = list.files(file.path(s2_dir, "include"), "\\.(h|hpp|cpp)$", full.names = TRUE, recursive = TRUE),
...
source_files <- tibble(
  path = list.files(file.path(s2_dir, "src"), "\\.(h|hpp|c|cpp)$", full.names = TRUE, recursive = TRUE),
