...
s2_iscollection <- function(x) {
  cpp_s2_iscollection(s2_geography(x))
}

#' @rdname s2_iscollection
#' @export
s2_dimension <- function(x) {
  cpp_s2_dimension(s2_geography(x))
}

#' @rdname s2_iscollection
#' @export
s2_numpoints <- function(x) {
  cpp_s2_numpoints(s2_geography(x))
}
...
s2_area <- function(x, radius = s2_earth_radius_meters()) {
  recycled <- recycle_common(s2_geography(x), radius)
...
#' @export
s2_x <- function(x) {
  cpp_s2_x(s2_geography(x))
}

#' @rdname s2_iscollection
#' @export
s2_y <- function(x) {
  cpp_s2_y(s2_geography(x))
}

#' @rdname s2_iscollection
#' @export
s2_distance <- function(x, y, radius = s2_earth_radius_meters()) {
  recycled <- recycle_common(s2_geography(x), s2_geography(y), radius)
